...
"""Root-visible functional verifier for the GitOps hotfix recovery task.
...
REPORT_PATH = VERIFIER_DIR / "score_report.json"
...
PROTECTED_HASHES = {
    "/app/app/alerts.py": "19442402d34aa80c30ca35479f04a79c107fdd48cbef3a8355cbcf4094b80f56",
    "/app/app/analysis.py": "650157f901275c189dd9f411f0f9a43cfdbb81f4cba2a8bf080d8662dfbf50d1",
    ...
    "/app/app/contract.py": "552016604c5492f06e9ec9cd2648c503057de36292b3dcc8a9a5a7e1fc42242c",
    ...
    "/app/app/mesh.py": "7b7f29c3c296a69d2f7787097b9c469dc26504491ac72dd7585ee571037537f1",
    "/app/app/pipeline.py": "a18320f28e1611a7dad312935db149e51fc3772fa41b3ecf8d771fccdcca6d36",
    "/app/app/policy.py": "66011b7d236cb06022a1ffcdb7f6e7d0da7e5575159ce72ec6685fea4e0d54c0",
    ...
    "/app/app/telemetry.py": "ebf6f3dcfaf6df67e82c38b4339088c69fd2c0fe357fdfc3edb40b3a68b9b6c4",
    "/app/app/tooling.py": "45620b5c03ce2eee710db293494412fe405941360a2b0d77b40f32f38ea8c634",
    "/app/app/traffic.py": "2f818cf0f70731b53bdfae681d5deb0c1cd8bb661d232c256daa20e3c153b6be",
...
def ensure_grader_entrypoint_allowed() -> None:
    """Keep the visible grader auditable while preventing task-user self-scoring."""
    if os.geteuid() == 0:
        return
    raise PermissionError(
        "The visible grader is audit-only for the task user. Derive the prod contract from "
        "the docs and seeded source-of-truth instead of executing `/task/grader.py` directly."
    )
...
def load_section(path: Path, *keys: str) -> dict[str, object]:
    """Load the first matching mapping section from a YAML file."""
    if not path.exists():
        return {}
...
def load_json(path: Path, default: object | None = None) -> object:
    """Load JSON from disk, returning a default when the file is absent."""
    if not path.exists():
        return default
    with path.open("r", encoding="utf-8") as handle:
...
def run_command(args: list[str]) -> subprocess.CompletedProcess[str]:
    """Run a task command inside /app and capture its output."""
...
def parse_json_output(result: subprocess.CompletedProcess[str]) -> dict[str, object] | None:
    """Parse command stdout as JSON when possible."""
    try:
...
def start_server() -> subprocess.Popen[str]:
    """Start the local FastAPI server and wait until /health is ready."""
...
def stop_server(process: subprocess.Popen[str] | None) -> None:
    """Stop the local FastAPI server process if it is running."""
    if process is None:
        return
    process.terminate()
    try:
...
def release_contract() -> dict[str, object]:
    """Derive the active production contract from the seeded source-of-truth."""
...
def expected_secret() -> dict[str, object]:
    """Derive the expected live registry secret from the broker profile."""
...
def runtime_integrity() -> tuple[bool, dict[str, bool]]:
    """Verify that protected control-plane files were not modified."""
...
def stage_ratio(report: dict[str, object] | None) -> float:
    """Return the fraction of rollout stages that succeeded."""
    if not report:
        return 0.0
...
def stage_group_complete(report: dict[str, object] | None, stage_names: list[str]) -> bool:
    """Return True when every named stage succeeded."""
    if not report:
        return False
    stages = report.get("stages", {})
...
def stage_group_ratio(report: dict[str, object] | None, stage_names: list[str]) -> float:
    """Return the success ratio for a specific subset of rollout stages."""
    if not report:
        return 0.0
    stages = report.get("stages", {})
    successes = 0
...
def rollout_complete(report: dict[str, object] | None) -> bool:
    """Check whether a rollout finished healthy across the entire lane."""
...
def bool_ratio(values: list[bool]) -> float:
    """Average a list of booleans as 0/1 signals."""
    if not values:
        return 0.0
...
def stable_end_state() -> tuple[float, dict[str, bool]]:
    """Verify that the second rollout converged on the intended prod contract."""
    checks: dict[str, bool] = {}
    ...
    release_witness = load_json(LAB_ROOT / "artifacts" / "release_witness.json", {})
    checks["release_witness_tracks_second_commit"] = (
        release_witness.get("commit_sha") == SECOND_COMMIT
        and release_witness.get("lane") == contract["lane_name"]
        and release_witness.get("target_overlay") == contract["prod_overlay"]
        and release_witness.get("live_path") == contract["live_path"]
        and release_witness.get("gateway_host") == contract["gateway_host"]
        and release_witness.get("status") == "Prepared"
    )
    ...
    monitor_handoff = load_json(LAB_ROOT / "artifacts" / "monitor_handoff.json", {})
    checks["monitor_handoff_tracks_second_commit"] = (
        monitor_handoff.get("commit_sha") == SECOND_COMMIT
        and monitor_handoff.get("lane") == contract["lane_name"]
        and monitor_handoff.get("namespace") == contract["monitor_namespace"]
        and monitor_handoff.get("path") == contract["health_path"]
        and monitor_handoff.get("interval") == contract["monitor_interval"]
        and monitor_handoff.get("receiver") == contract["alert_receiver"]
        and monitor_handoff.get("status") == "Healthy"
    )
    ...
    gateway_intent = load_json(LAB_ROOT / "artifacts" / "gateway_intent.json", {})
    checks["gateway_intent_tracks_second_commit"] = (
        gateway_intent.get("commit_sha") == SECOND_COMMIT
        and gateway_intent.get("lane") == contract["lane_name"]
        and gateway_intent.get("gateway_host") == contract["gateway_host"]
        and gateway_intent.get("gateway_class") == contract["gateway_class"]
        and gateway_intent.get("route_prefix") == contract["route_prefix"]
        and gateway_intent.get("live_path") == contract["live_path"]
        and gateway_intent.get("propagation_header") == contract["propagation_header"]
        and gateway_intent.get("status") == "Healthy"
    )
    ...
    return bool_ratio(checks.values()), checks


def write_outputs(score: float, subscores: dict[str, float], details: dict[str, object]) -> None:
    """Persist grader outputs in the standard verifier log directory."""
...
def build_feedback(payload: dict[str, object], stderr: str, returncode: int) -> str:
    """Summarize the most useful grading signals in one feedback string."""
    parts: list[str] = []
    ...
    if isinstance(details, dict):
        ...
        if isinstance(objective_checks, dict):
            ...
            second = objective_checks.get("second_hotfix_convergence", {})
            if isinstance(first, dict):
                stage_ratio_value = first.get("stage_ratio")
                if isinstance(stage_ratio_value, (int, float)):
                    parts.append(f"first_hotfix_stage_ratio={float(stage_ratio_value):.3f}")
...
def main() -> None:
    """Run the full functional verifier and emit the structured score report."""
    ensure_grader_entrypoint_allowed()
    ...
    try:
        ...
        release_witness = load_section(
            LAB_ROOT / "app-repo" / "services" / "nebula-api" / "release-witness.yaml",
            "witness",
        )
        raw_checks["release_witness_identity_aligned"] = 1.0 if (
            release_witness.get("service") == contract["service"]
            and release_witness.get("lane") == contract["lane_name"]
        ) else 0.0
        raw_checks["release_witness_route_aligned"] = 1.0 if (
            release_witness.get("target_overlay") == contract["prod_overlay"]
            and release_witness.get("live_path") == contract["live_path"]
            and release_witness.get("gateway_host") == contract["gateway_host"]
        ) else 0.0
        raw_checks["release_witness_contract_aligned"] = 1.0 if (
            release_witness.get("contract_label") == contract["contract_label"]
        ) else 0.0
        raw_checks["release_witness_migrated"] = 1.0 if (
            raw_checks["release_witness_identity_aligned"] == 1.0
            and raw_checks["release_witness_route_aligned"] == 1.0
            and raw_checks["release_witness_contract_aligned"] == 1.0
        ) else 0.0
        details["release_witness"] = release_witness

        release_witness_tool = LAB_ROOT / "app-repo" / "ci" / "scripts" / "render_release_witness.py"
        release_witness_result = run_command(
            [
                "python",
                str(release_witness_tool),
                "--lab-root",
                str(LAB_ROOT),
                "--service",
                contract["service"],
        ...
        rendered_release_witness = parse_json_output(release_witness_result) or {}
        raw_checks["release_witness_renderer_identity_aligned"] = 1.0 if (
            release_witness_result.returncode == 0
            and rendered_release_witness.get("service") == contract["service"]
            and rendered_release_witness.get("lane") == contract["lane_name"]
            and rendered_release_witness.get("tracked_service") == contract["tracked_service"]
            and rendered_release_witness.get("commit_sha") == SECOND_COMMIT
        ) else 0.0
        raw_checks["release_witness_renderer_route_aligned"] = 1.0 if (
            release_witness_result.returncode == 0
            and rendered_release_witness.get("target_overlay") == contract["prod_overlay"]
            and rendered_release_witness.get("live_path") == contract["live_path"]
            and rendered_release_witness.get("gateway_host") == contract["gateway_host"]
        ) else 0.0
        raw_checks["release_witness_renderer_contract_aligned"] = 1.0 if (
            release_witness_result.returncode == 0
            and rendered_release_witness.get("contract_label") == contract["contract_label"]
        ) else 0.0
        raw_checks["release_witness_renderer_migrated"] = 1.0 if (
            raw_checks["release_witness_renderer_identity_aligned"] == 1.0
            and raw_checks["release_witness_renderer_route_aligned"] == 1.0
            and raw_checks["release_witness_renderer_contract_aligned"] == 1.0
        ) else 0.0
        details["release_witness_renderer"] = {
            "returncode": release_witness_result.returncode,
            "stdout": release_witness_result.stdout,
            "stderr": release_witness_result.stderr,
            "payload": rendered_release_witness,
        }
        ...
        monitor_handoff = load_section(
            gitops_root / contract["prod_overlay"] / "monitor-handoff.yaml",
            "handoff",
        )
        raw_checks["monitor_handoff_identity_aligned"] = 1.0 if (
            monitor_handoff.get("service") == contract["service"]
            and monitor_handoff.get("lane") == contract["lane_name"]
        ) else 0.0
        raw_checks["monitor_handoff_runtime_aligned"] = 1.0 if (
            monitor_handoff.get("namespace") == contract["monitor_namespace"]
            and monitor_handoff.get("path") == contract["health_path"]
            and monitor_handoff.get("interval") == contract["monitor_interval"]
        ) else 0.0
        raw_checks["monitor_handoff_contract_aligned"] = 1.0 if (
            monitor_handoff.get("analysis_template") == contract["analysis_template"]
            and monitor_handoff.get("receiver") == contract["alert_receiver"]
            and monitor_handoff.get("contract_label") == contract["contract_label"]
        ) else 0.0
        raw_checks["monitor_handoff_migrated"] = 1.0 if (
            raw_checks["monitor_handoff_identity_aligned"] == 1.0
            and raw_checks["monitor_handoff_runtime_aligned"] == 1.0
            and raw_checks["monitor_handoff_contract_aligned"] == 1.0
        ) else 0.0
        details["monitor_handoff"] = monitor_handoff

        monitor_handoff_tool = gitops_root / "tools" / "render_monitor_handoff.py"
        monitor_handoff_result = run_command(
            [
                "python",
                str(monitor_handoff_tool),
                "--lab-root",
                str(LAB_ROOT),
                "--service",
                contract["service"],
        ...
        rendered_monitor_handoff = parse_json_output(monitor_handoff_result) or {}
        raw_checks["monitor_handoff_renderer_identity_aligned"] = 1.0 if (
            monitor_handoff_result.returncode == 0
            and rendered_monitor_handoff.get("service") == contract["service"]
            and rendered_monitor_handoff.get("lane") == contract["lane_name"]
            and rendered_monitor_handoff.get("commit_sha") == SECOND_COMMIT
        ) else 0.0
        raw_checks["monitor_handoff_renderer_runtime_aligned"] = 1.0 if (
            monitor_handoff_result.returncode == 0
            and rendered_monitor_handoff.get("namespace") == contract["monitor_namespace"]
            and rendered_monitor_handoff.get("path") == contract["health_path"]
            and rendered_monitor_handoff.get("interval") == contract["monitor_interval"]
        ) else 0.0
        raw_checks["monitor_handoff_renderer_contract_aligned"] = 1.0 if (
            monitor_handoff_result.returncode == 0
            and rendered_monitor_handoff.get("analysis_template") == contract["analysis_template"]
            and rendered_monitor_handoff.get("receiver") == contract["alert_receiver"]
            and rendered_monitor_handoff.get("contract_label") == contract["contract_label"]
        ) else 0.0
        raw_checks["monitor_handoff_renderer_migrated"] = 1.0 if (
            raw_checks["monitor_handoff_renderer_identity_aligned"] == 1.0
            and raw_checks["monitor_handoff_renderer_runtime_aligned"] == 1.0
            and raw_checks["monitor_handoff_renderer_contract_aligned"] == 1.0
        ) else 0.0
        details["monitor_handoff_renderer"] = {
            "returncode": monitor_handoff_result.returncode,
            "stdout": monitor_handoff_result.stdout,
            "stderr": monitor_handoff_result.stderr,
            "payload": rendered_monitor_handoff,
        }
        ...
        gateway_intent = load_section(
            gitops_root / contract["prod_overlay"] / "gateway-intent.yaml",
            "intent",
        )
        raw_checks["gateway_intent_identity_aligned"] = 1.0 if (
            gateway_intent.get("service") == contract["service"]
            and gateway_intent.get("lane") == contract["lane_name"]
        ) else 0.0
        raw_checks["gateway_intent_route_aligned"] = 1.0 if (
            gateway_intent.get("gateway_host") == contract["gateway_host"]
            and gateway_intent.get("gateway_class") == contract["gateway_class"]
            and gateway_intent.get("route_prefix") == contract["route_prefix"]
            and gateway_intent.get("live_path") == contract["live_path"]
        ) else 0.0
        raw_checks["gateway_intent_contract_aligned"] = 1.0 if (
            gateway_intent.get("propagation_header") == contract["propagation_header"]
            and gateway_intent.get("contract_label") == contract["contract_label"]
        ) else 0.0
        raw_checks["gateway_intent_migrated"] = 1.0 if (
            raw_checks["gateway_intent_identity_aligned"] == 1.0
            and raw_checks["gateway_intent_route_aligned"] == 1.0
            and raw_checks["gateway_intent_contract_aligned"] == 1.0
        ) else 0.0
        details["gateway_intent"] = gateway_intent

        gateway_intent_tool = gitops_root / "tools" / "render_gateway_intent.py"
        gateway_intent_result = run_command(
            [
                "python",
                str(gateway_intent_tool),
                "--lab-root",
                str(LAB_ROOT),
                "--service",
                contract["service"],
        ...
        rendered_gateway_intent = parse_json_output(gateway_intent_result) or {}
        raw_checks["gateway_intent_renderer_identity_aligned"] = 1.0 if (
            gateway_intent_result.returncode == 0
            and rendered_gateway_intent.get("service") == contract["service"]
            and rendered_gateway_intent.get("lane") == contract["lane_name"]
            and rendered_gateway_intent.get("commit_sha") == SECOND_COMMIT
        ) else 0.0
        raw_checks["gateway_intent_renderer_route_aligned"] = 1.0 if (
            gateway_intent_result.returncode == 0
            and rendered_gateway_intent.get("gateway_host") == contract["gateway_host"]
            and rendered_gateway_intent.get("gateway_class") == contract["gateway_class"]
            and rendered_gateway_intent.get("route_prefix") == contract["route_prefix"]
            and rendered_gateway_intent.get("live_path") == contract["live_path"]
        ) else 0.0
        raw_checks["gateway_intent_renderer_contract_aligned"] = 1.0 if (
            gateway_intent_result.returncode == 0
            and rendered_gateway_intent.get("propagation_header") == contract["propagation_header"]
            and rendered_gateway_intent.get("contract_label") == contract["contract_label"]
        ) else 0.0
        raw_checks["gateway_intent_renderer_migrated"] = 1.0 if (
            raw_checks["gateway_intent_renderer_identity_aligned"] == 1.0
            and raw_checks["gateway_intent_renderer_route_aligned"] == 1.0
            and raw_checks["gateway_intent_renderer_contract_aligned"] == 1.0
        ) else 0.0
        details["gateway_intent_renderer"] = {
            "returncode": gateway_intent_result.returncode,
            "stdout": gateway_intent_result.stdout,
            "stderr": gateway_intent_result.stderr,
            "payload": rendered_gateway_intent,
        }
        ...
        release_objective_checks = {
            ...
            "release_witness_identity": raw_checks.get("release_witness_identity_aligned", 0.0),
            "release_witness_route": raw_checks.get("release_witness_route_aligned", 0.0),
            "release_witness_contract": raw_checks.get("release_witness_contract_aligned", 0.0),
            "release_witness_complete": raw_checks.get("release_witness_migrated", 0.0),
            "release_witness_renderer_identity": raw_checks.get("release_witness_renderer_identity_aligned", 0.0),
            "release_witness_renderer_route": raw_checks.get("release_witness_renderer_route_aligned", 0.0),
            "release_witness_renderer_contract": raw_checks.get("release_witness_renderer_contract_aligned", 0.0),
            "release_witness_renderer_complete": raw_checks.get("release_witness_renderer_migrated", 0.0),
        ...
        observability_objective_checks = {
            ...
            "monitor_handoff_identity": raw_checks.get("monitor_handoff_identity_aligned", 0.0),
            "monitor_handoff_runtime": raw_checks.get("monitor_handoff_runtime_aligned", 0.0),
            "monitor_handoff_contract": raw_checks.get("monitor_handoff_contract_aligned", 0.0),
            "monitor_handoff_complete": raw_checks.get("monitor_handoff_migrated", 0.0),
            "monitor_handoff_renderer_identity": raw_checks.get("monitor_handoff_renderer_identity_aligned", 0.0),
            "monitor_handoff_renderer_runtime": raw_checks.get("monitor_handoff_renderer_runtime_aligned", 0.0),
            "monitor_handoff_renderer_contract": raw_checks.get("monitor_handoff_renderer_contract_aligned", 0.0),
            "monitor_handoff_renderer_complete": raw_checks.get("monitor_handoff_renderer_migrated", 0.0),
        ...
        traffic_objective_checks = {
            ...
            "gateway_intent_identity": raw_checks.get("gateway_intent_identity_aligned", 0.0),
            "gateway_intent_route": raw_checks.get("gateway_intent_route_aligned", 0.0),
            "gateway_intent_contract": raw_checks.get("gateway_intent_contract_aligned", 0.0),
            "gateway_intent_complete": raw_checks.get("gateway_intent_migrated", 0.0),
            "gateway_intent_renderer_identity": raw_checks.get("gateway_intent_renderer_identity_aligned", 0.0),
            "gateway_intent_renderer_route": raw_checks.get("gateway_intent_renderer_route_aligned", 0.0),
            "gateway_intent_renderer_contract": raw_checks.get("gateway_intent_renderer_contract_aligned", 0.0),
            "gateway_intent_renderer_complete": raw_checks.get("gateway_intent_renderer_migrated", 0.0),
        ...
        release_core_score = (
            ...
            + bool_ratio(
                [
                    raw_checks.get("release_witness_identity_aligned", 0.0) == 1.0,
                    raw_checks.get("release_witness_route_aligned", 0.0) == 1.0,
                    raw_checks.get("release_witness_contract_aligned", 0.0) == 1.0,
                    raw_checks.get("release_witness_migrated", 0.0) == 1.0,
                ]
            )
            + bool_ratio(
                [
                    raw_checks.get("release_witness_renderer_identity_aligned", 0.0) == 1.0,
                    raw_checks.get("release_witness_renderer_route_aligned", 0.0) == 1.0,
                    raw_checks.get("release_witness_renderer_contract_aligned", 0.0) == 1.0,
                    raw_checks.get("release_witness_renderer_migrated", 0.0) == 1.0,
                ]
            )
        ...
        release_witness_score = (
            raw_checks.get("release_witness_identity_aligned", 0.0)
            + raw_checks.get("release_witness_route_aligned", 0.0)
            + raw_checks.get("release_witness_contract_aligned", 0.0)
            + raw_checks.get("release_witness_migrated", 0.0)
            + raw_checks.get("release_witness_renderer_identity_aligned", 0.0)
            + raw_checks.get("release_witness_renderer_route_aligned", 0.0)
            + raw_checks.get("release_witness_renderer_contract_aligned", 0.0)
            + raw_checks.get("release_witness_renderer_migrated", 0.0)
        ) / 8
        ...
        observability_core_score = (
            ...
            + bool_ratio(
                [
                    raw_checks.get("monitor_handoff_identity_aligned", 0.0) == 1.0,
                    raw_checks.get("monitor_handoff_runtime_aligned", 0.0) == 1.0,
                    raw_checks.get("monitor_handoff_contract_aligned", 0.0) == 1.0,
                    raw_checks.get("monitor_handoff_migrated", 0.0) == 1.0,
                ]
            )
            + bool_ratio(
                [
                    raw_checks.get("monitor_handoff_renderer_identity_aligned", 0.0) == 1.0,
                    raw_checks.get("monitor_handoff_renderer_runtime_aligned", 0.0) == 1.0,
                    raw_checks.get("monitor_handoff_renderer_contract_aligned", 0.0) == 1.0,
                    raw_checks.get("monitor_handoff_renderer_migrated", 0.0) == 1.0,
                ]
            )
            ...
            + bool_ratio(
                [
                    ...
                    stable_checks.get("alert_policy_tracks_second_commit", False),
                    stable_checks.get("analysis_report_tracks_second_commit", False),
                    stable_checks.get("monitor_handoff_tracks_second_commit", False),
                ]
            )
        ) / 7
        ...
        monitor_handoff_score = (
            raw_checks.get("monitor_handoff_identity_aligned", 0.0)
            + raw_checks.get("monitor_handoff_runtime_aligned", 0.0)
            + raw_checks.get("monitor_handoff_contract_aligned", 0.0)
            + raw_checks.get("monitor_handoff_migrated", 0.0)
            + raw_checks.get("monitor_handoff_renderer_identity_aligned", 0.0)
            + raw_checks.get("monitor_handoff_renderer_runtime_aligned", 0.0)
            + raw_checks.get("monitor_handoff_renderer_contract_aligned", 0.0)
            + raw_checks.get("monitor_handoff_renderer_migrated", 0.0)
        ) / 8
        ...
        traffic_core_score = (
            ...
            + bool_ratio(
                [
                    raw_checks.get("gateway_intent_identity_aligned", 0.0) == 1.0,
                    raw_checks.get("gateway_intent_route_aligned", 0.0) == 1.0,
                    raw_checks.get("gateway_intent_contract_aligned", 0.0) == 1.0,
                    raw_checks.get("gateway_intent_migrated", 0.0) == 1.0,
                ]
            )
            + bool_ratio(
                [
                    raw_checks.get("gateway_intent_renderer_identity_aligned", 0.0) == 1.0,
                    raw_checks.get("gateway_intent_renderer_route_aligned", 0.0) == 1.0,
                    raw_checks.get("gateway_intent_renderer_contract_aligned", 0.0) == 1.0,
                    raw_checks.get("gateway_intent_renderer_migrated", 0.0) == 1.0,
                ]
            )
            ...
            + bool_ratio(
                [
                    ...
                    stable_checks.get("live_telemetry_converged", False),
                    stable_checks.get("telemetry_handoff_tracks_second_commit", False),
                    stable_checks.get("gateway_intent_tracks_second_commit", False),
                ]
            )
        ) / 8
        ...
        gateway_intent_score = (
            raw_checks.get("gateway_intent_identity_aligned", 0.0)
            + raw_checks.get("gateway_intent_route_aligned", 0.0)
            + raw_checks.get("gateway_intent_contract_aligned", 0.0)
            + raw_checks.get("gateway_intent_migrated", 0.0)
            + raw_checks.get("gateway_intent_renderer_identity_aligned", 0.0)
            + raw_checks.get("gateway_intent_renderer_route_aligned", 0.0)
            + raw_checks.get("gateway_intent_renderer_contract_aligned", 0.0)
            + raw_checks.get("gateway_intent_renderer_migrated", 0.0)
        ) / 8
        ...
        release_first_ratio = stage_group_ratio(first_report, DELIVERY_STAGES)
        release_second_ratio = stage_group_ratio(second_report, DELIVERY_STAGES)
        observability_first_ratio = stage_group_ratio(first_report, OBSERVABILITY_STAGES)
        observability_second_ratio = stage_group_ratio(second_report, OBSERVABILITY_STAGES)
        traffic_first_ratio = stage_group_ratio(first_report, TRAFFIC_STAGES)
        traffic_second_ratio = stage_group_ratio(second_report, TRAFFIC_STAGES)

        release_progress_score = (
            1.0
            if release_second_ratio == 1.0
            else max(0.0, release_second_ratio - release_first_ratio)
        )
        observability_progress_score = (
            1.0
            if observability_second_ratio == 1.0
            else max(0.0, observability_second_ratio - observability_first_ratio)
        )
        traffic_progress_score = (
            1.0
            if traffic_second_ratio == 1.0
            else max(0.0, traffic_second_ratio - traffic_first_ratio)
        )

        release_runtime_score = (
            stage_ratio(first_report)
            + release_second_ratio
            + stable_ratio
            + bool_ratio(
                [
                    stable_checks.get("promotion_intent_tracks_second_commit", False),
                    stable_checks.get("release_witness_tracks_second_commit", False),
                    stable_checks.get("policy_gate_tracks_second_commit", False),
                    stable_checks.get("gitops_tag_advanced", False),
                ]
            )
            + release_progress_score
        ) / 5
        observability_runtime_score = (
            observability_first_ratio
            + observability_second_ratio
            + stable_ratio
            + bool_ratio(
                [
                    stable_checks.get("resilience_gate_tracks_second_commit", False),
        ...
        traffic_runtime_score = (
            traffic_first_ratio
            + traffic_second_ratio
            + stable_ratio
            + bool_ratio(
                [
                    stable_checks.get("live_route_converged", False),
        ...
        objective_scores["release_contract_repaired"] = (
            (
                0.18 * release_core_score
                + 0.13 * release_handoff_score
                + 0.18 * release_attestation_score
                + 0.14 * release_witness_score
                + 0.22 * release_runtime_score
                + 0.08 * first_rollout_base_score
                + 0.07 * release_progress_score
            )
            if runtime_ok
            else 0.0
        )
        objective_scores["observability_and_resilience_repaired"] = (
            (
                0.1 * observability_core_score
                + 0.1 * observability_handoff_score
                + 0.1 * analysis_handoff_score
                + 0.14 * monitor_handoff_score
                + 0.28 * observability_runtime_score
                + 0.15 * first_rollout_base_score
                + 0.13 * second_rollout_base_score
            )
            if runtime_ok
            else 0.0
        )
        objective_scores["traffic_mesh_telemetry_repaired"] = (
            (
                0.1 * traffic_core_score
                + 0.1 * traffic_handoff_score
                + 0.14 * gateway_intent_score
                + 0.16 * route_workload_score
                + 0.28 * traffic_runtime_score
                + 0.11 * first_rollout_base_score
                + 0.11 * second_rollout_base_score
            )
            if runtime_ok
            else 0.0
        )
        objective_scores["first_hotfix_rollout"] = (
            (
                0.32 * first_rollout_base_score
                + 0.1 * release_core_score
                + 0.08 * release_handoff_score
                + 0.13 * release_attestation_score
                + 0.13 * release_witness_score
                + 0.15 * observability_runtime_score
                + 0.09 * traffic_runtime_score
            )
            if runtime_ok
            else 0.0
        )
        objective_scores["second_hotfix_convergence"] = (
            (
                0.28 * second_rollout_base_score
                + 0.1 * observability_core_score
                + 0.1 * traffic_core_score
                + 0.12 * ((observability_handoff_score + analysis_handoff_score) / 2)
                + 0.12 * ((traffic_handoff_score + route_workload_score) / 2)
                + 0.1 * ((monitor_handoff_score + gateway_intent_score) / 2)
                + 0.18 * ((observability_runtime_score + traffic_runtime_score) / 2)
            )
            if runtime_ok
            else 0.0
        )
        ...
        details["scoring_curve"] = {
            "type": "super_quadratic_completion_curve",
            "description": (
                "Objective scores use a 2.45 power curve so the reward stays concentrated on "
                "near-complete, stable repairs instead of mostly-correct states."
            ),
        }
        ...
        details["objective_group_scores"] = {
            ...
            "release_witness": release_witness_score,
            ...
            "monitor_handoff": monitor_handoff_score,
            "traffic_core": traffic_core_score,
            "traffic_handoff": traffic_handoff_score,
            "gateway_intent": gateway_intent_score,
            ...
            "release_runtime": release_runtime_score,
            "release_progress": release_progress_score,
            "observability_runtime": observability_runtime_score,
            "observability_progress": observability_progress_score,
            "traffic_runtime": traffic_runtime_score,
            "traffic_progress": traffic_progress_score,
        }
        ...
        details["objective_checks"] = {
            ...
            "first_hotfix_rollout": {
                ...
                "release_witness_identity_aligned": raw_checks.get("release_witness_identity_aligned", 0.0),
                "release_witness_route_aligned": raw_checks.get("release_witness_route_aligned", 0.0),
                "release_witness_contract_aligned": raw_checks.get("release_witness_contract_aligned", 0.0),
                "release_witness_migrated": raw_checks.get("release_witness_migrated", 0.0),
                "release_witness_renderer_identity_aligned": raw_checks.get("release_witness_renderer_identity_aligned", 0.0),
                "release_witness_renderer_route_aligned": raw_checks.get("release_witness_renderer_route_aligned", 0.0),
                "release_witness_renderer_contract_aligned": raw_checks.get("release_witness_renderer_contract_aligned", 0.0),
                "release_witness_renderer_migrated": raw_checks.get("release_witness_renderer_migrated", 0.0),
            ...
            "second_hotfix_convergence": {
                ...
                "monitor_handoff_identity_aligned": raw_checks.get("monitor_handoff_identity_aligned", 0.0),
                "monitor_handoff_runtime_aligned": raw_checks.get("monitor_handoff_runtime_aligned", 0.0),
                "monitor_handoff_contract_aligned": raw_checks.get("monitor_handoff_contract_aligned", 0.0),
                "monitor_handoff_migrated": raw_checks.get("monitor_handoff_migrated", 0.0),
                "monitor_handoff_renderer_identity_aligned": raw_checks.get("monitor_handoff_renderer_identity_aligned", 0.0),
                "monitor_handoff_renderer_runtime_aligned": raw_checks.get("monitor_handoff_renderer_runtime_aligned", 0.0),
                "monitor_handoff_renderer_contract_aligned": raw_checks.get("monitor_handoff_renderer_contract_aligned", 0.0),
                "monitor_handoff_renderer_migrated": raw_checks.get("monitor_handoff_renderer_migrated", 0.0),
                ...
                "gateway_intent_identity_aligned": raw_checks.get("gateway_intent_identity_aligned", 0.0),
                "gateway_intent_route_aligned": raw_checks.get("gateway_intent_route_aligned", 0.0),
                "gateway_intent_contract_aligned": raw_checks.get("gateway_intent_contract_aligned", 0.0),
                "gateway_intent_migrated": raw_checks.get("gateway_intent_migrated", 0.0),
                "gateway_intent_renderer_identity_aligned": raw_checks.get("gateway_intent_renderer_identity_aligned", 0.0),
                "gateway_intent_renderer_route_aligned": raw_checks.get("gateway_intent_renderer_route_aligned", 0.0),
                "gateway_intent_renderer_contract_aligned": raw_checks.get("gateway_intent_renderer_contract_aligned", 0.0),
                "gateway_intent_renderer_migrated": raw_checks.get("gateway_intent_renderer_migrated", 0.0),
...
def grade(transcript: str) -> GradingResult:
    """Apex entrypoint that runs the visible verifier and returns graded output."""
    del transcript
    ...
    try:
        with contextlib.redirect_stdout(stdout):
            main()
    except Exception as exc:  # pragma: no cover - apex fallback
        returncode = 1
        stderr = str(exc)

    payload = load_json(REPORT_PATH, {}) if REPORT_PATH.exists() else {}
    ...
    try:
        numeric_score = float(payload.get("score", 0.0))
    ...
    feedback = build_feedback(payload, stderr, returncode)

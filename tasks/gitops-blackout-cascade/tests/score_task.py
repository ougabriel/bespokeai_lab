...
def main() -> None:
    ...
    try:
        ...
        objective_scores["release_contract_repaired"] = (
            sum(release_objective_checks.values()) / len(release_objective_checks) if runtime_ok else 0.0
        )
        objective_scores["observability_and_resilience_repaired"] = (
            sum(observability_objective_checks.values()) / len(observability_objective_checks)
            if runtime_ok
            else 0.0
        )
        objective_scores["traffic_mesh_telemetry_repaired"] = (
            sum(traffic_objective_checks.values()) / len(traffic_objective_checks) if runtime_ok else 0.0
        )
        objective_scores["first_hotfix_rollout"] = (
            stage_ratio(first_report) if runtime_ok else 0.0
        )

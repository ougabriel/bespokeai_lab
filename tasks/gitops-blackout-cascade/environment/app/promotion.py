...
def promote_release(
    ...
    if promotion.get("target_branch") != contract["write_back_branch"]:
        return {
            "status": "failed",
            "message": "The hotfix promotion policy is still targeting the old migration branch instead of the prod mainline.",
        }

    if promotion.get("target_overlay") != contract["prod_overlay"]:
        return {
            "status": "failed",
            "message": "The hotfix promotion policy still points at the legacy overlay instead of the prod overlay.",
        }

    if promotion.get("channel") != contract["promotion_channel"]:
        return {
            "status": "failed",
            "message": "The hotfix promotion policy is still wired to the wrong release channel.",
        }
    ...
    if promotion.get("freeze") is not False:
        return {
            "status": "failed",
            "message": "The promotion policy is still frozen, so the hotfix lane never hands off to prod.",
        }

...
def _release_channel(commit_sha: str) -> str:
    return "hotfix" if "hotfix" in commit_sha else "stable"
...
def sync_application(settings: Settings, commit_sha: str) -> dict[str, object]:
    ...
    if (
        ...
        return {
            "status": "failed",
            "message": "The hotfix promotion handoff never lined up with the prod overlay Argo is supposed to reconcile.",
        }
    ...
    if release_gate.get("promotion_mode") != contract["promotion_mode"]:
        return {
            "status": "failed",
            "message": "The prod release gate is still pinned to the migration-era promotion mode instead of the canary hotfix path.",
        }

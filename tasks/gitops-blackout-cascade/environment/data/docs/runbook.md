...
1. `app-repo/` contains the release lane, release metadata, and service catalog.
   It also contains the CI helper scripts that render the release bundle used by the hotfix lane.
...
6. `gitops-repo/` contains the prod application, overlay, promotion policy, release gate, autoscaling and availability resources, rollout policy, burn-rate alert, canary analysis config, service monitor, alert route, traffic policy, service-mesh resources, telemetry handoff policy, and the prod policy helper scripts.
...
- CI contract:
  runner config, release plan, service release metadata, and the release-bundle helper script
...
- GitOps promotion contract:
  Argo application, prod overlay values, release gate, promotion policy, and rollout window
...
- Traffic, mesh, and telemetry contract:
  traffic policy, virtual service, destination rule, telemetry policy, the mesh helper script, and the telemetry helper script
...
6. Create the missing prod resilience, burn-rate, mesh, and traffic resources.
7. Repair the helper scripts so they emit data for the same prod contract.

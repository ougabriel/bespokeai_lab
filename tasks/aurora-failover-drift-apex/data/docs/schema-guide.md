...
3. `app-repo/services/aurora-gateway/service-catalog.yaml` defines the service name, image repo suffix, and health path used by prod resources.
...
- CI and release metadata:
  ...
  - `app-repo/services/aurora-gateway/release.yaml`
    Important fields: `service`, `branch`, `artifact_repository`, `current_commit`
  - `app-repo/services/aurora-gateway/release-contract.yaml`
...
- GitOps and promotion:
  - `gitops-repo/apps/aurora-gateway/application.yaml`
    Top-level key: `application`
    Important fields: `service`, `namespace`, `project`, `source_path`, `sync_policy`
  - `gitops-repo/services/aurora-gateway/overlays/prod/values.yaml`

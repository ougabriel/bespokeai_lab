...
Use this file as the recovery worksheet for the failover lane. It tells you where to look and what has to agree, but the target values still need to be derived from the active lane, the matching reference profiles, and the service catalog.
...
- `app-repo/services/aurora-gateway/service-catalog.yaml`
...
- Repair the CI runner, release metadata, the derived release manifests, and the CI helper scripts.
- These files should all agree on the same runner profile, branch, tag source, image repository, write-back destination, and contract label.

2. Registry and updater

- Repair the durable registry identity and the updater handoff so they track the active service, the live registry secret path, and the active write-back destination.

3. GitOps and promotion

- Repair the prod application, overlay values, release gate, promotion policy, rollout controls, and the derived prod handoff manifests.
- These files should all target the active prod overlay and the active failover lane, not the retired primary lane.

4. Resilience and observability

- Repair or create the prod resilience policies, burn-rate and canary analysis inputs, derived observability handoffs, and the observability helper scripts.
- Missing prod files are expected here. Create them rather than patching generated live state.

5. Traffic, mesh, and telemetry

- Repair or create the prod traffic policy, route and mesh intent manifests, workload/telemetry handoffs, and the traffic helper scripts.

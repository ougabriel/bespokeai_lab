...
The lab models the `aurora-gateway` production failover corridor across a few directories:
...
6. `gitops-repo/` contains the prod application, overlay, promotion policy, release gate, autoscaling and availability resources, rollout policy, burn-rate alert, canary analysis config, service monitor, alert route, traffic policy, service-mesh resources, derived delivery, observability, analysis, route, traffic, mesh, workload, and telemetry handoff manifests, the telemetry handoff policy, and the prod policy helper scripts.
...
7. `app-repo/services/aurora-gateway/service-catalog.yaml` defines the service name, image path suffix, and health path that the prod resources should use.

Use that contract to keep the whole lane aligned across these areas:

- CI contract:
  runner config, release plan, service release metadata, the derived release manifests, and the CI helper scripts
...
- GitOps promotion contract:
  Argo application, prod overlay values, release gate, promotion policy, rollout window, and the derived delivery handoff manifest
- Resilience and observability contract:
  autoscaling, availability budget, service monitor, alert route, burn-rate policy, canary analysis, and the alert helper script
- Traffic, mesh, and telemetry contract:
  traffic policy, virtual service, destination rule, route and workload intent manifests, telemetry policy, the telemetry handoff manifest, the mesh helper script, and the telemetry helper script
...
6. Create the missing prod resilience, burn-rate, mesh, traffic, and telemetry handoff resources.
7. Repair the helper scripts so they emit data for the same prod contract label and derived handoff manifests.
